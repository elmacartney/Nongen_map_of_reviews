...
my.cols <- (c("#BEAED4","#FFFF99" ,"#386CB0","#7FC97F","#FDC086","#F0027F")) #- ggplot seems to be randomly rearranging these in the plot
...
### Fig. 5B - Type of environmental exposure
...
Fig5b <- ggplot(percent_F0_env, aes(x = F0_env_code, y = percent)) + 
  geom_col(aes(fill = discipline_code), width = 0.7) + 
  coord_flip() + 
  scale_y_continuous(name = "Percent") +
  geom_text(position = position_stack(vjust = 0.5), aes(fill = discipline_code, label = percent)) +
...
### Fig. 5C - Descendant generation within F0 timing
