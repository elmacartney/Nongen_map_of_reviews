...
### Fig. 4C - Inter-vs trans- by transmission mode
...
Fig4c <- ggplot(percent_inter_vs_trans_transmission, aes(x = inter_vs_trans_code, y = percent)) + 
...
### Fig. 4D - Match vs mismatch inter- vs trans- definition
...
Fig4d <- ggplot(percent_terminology, aes(x = terminology_code, y = percent)) + 
  coord_flip() +  
  scale_y_continuous(name = "Percent") +
  geom_text(position = position_stack(vjust = 0.5), aes(fill = discipline_code, label = percent)) +
   theme_classic() +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Terminology matches definition in Fig. 1", fill = "Discipline")

Fig4d

#TODO why are bars not descending order
```
...
# TODO remove legend - have Fig. 3 with colours for each discipline
```